...
// performance-marketing teams actually ask. The facade reveals these on a timer to feel like
// streaming; the content is deterministic, cited, and honest about confidence.
//
// Every answer reasons across more than one data source (margin + ads + on-site
// behaviour + period-over-period) — the whole point: no single dashboard number
// answers these.
//
// SCOPE: bounded by what the app surfaces. Inventory, reorder and cash-runway
// answers were removed along with those features — a fallback that still
// answered them would contradict the UI, the tools and the live agent.
...
// Freshness reads the connector rather than asserting a fixed age. "8 min
// ago" printed forever was the same class of defect as the frozen alert
// timestamps: an age that never changes reads as a mock.
const ORDERS_FRESHNESS = () => `Computed from orders synced ${timeAgo(demoLastSync())}`;
...
const ANSWERS = [
  {
    ...
    citations: [{ label: "Insights → waterfall", href: "/insights" }, { label: "Products", href: "/products" }],
    confidence: "High", freshness: ORDERS_FRESHNESS(),
  },
  {
    id: "losing",
    ...
    citations: [{ label: "Recovery Slides → breakdown", href: "/products/p6" }, { label: "Add COGS", href: "/products" }],
    confidence: "High", freshness: ORDERS_FRESHNESS(),
  },
  {
    id: "ads-overall",
    ...
    citations: [{ label: "Insights · Channels", href: "/insights?view=channels" }],
  ...
  {
    id: "scale-cloud7",
    match: ["scale cloud", "afford to scale", "scale my best", "scale cloud 7", "afford to scale cloud", "put more budget", "more ad budget into"],
    steps: ["Checking Cloud 7 margin & marginal CM-ROAS…", "Checking how much of its demand ads actually drive…"],
    answer:
      "Yes, with a caveat about headroom — **Cloud 7 is a 61% CM1 Hero at 4.1× CM-ROAS**, comfortably above break-even. The limit is not margin, it is how much of its demand is actually addressable: ads drive only part of its orders, so budget added past that point buys clicks from people who were already going to buy.",
    bullets: ["Scale Meta ASC on Cloud 7 — it's the channel carrying it", "Step the budget, don't double it: CAC rises with reach", "Re-check CM-ROAS after a week; scale again only if it holds above 2×"],
    metrics: [{ label: "CM1", value: "61%" }, { label: "CM-ROAS", value: "4.1×" }, { label: "Verdict", value: "Scale" }],
    citations: [{ label: "Cloud 7 → breakdown", href: "/products/p2" }, { label: "Insights · Channels", href: "/insights?view=channels" }],
    confidence: "High on margin, Medium on headroom", freshness: "CAC at scale is an estimate — treat the top end as a ceiling",
  },
  {
    id: "discount",
  ...
  },
  {
    ...
    citations: [{ label: "Insights · Channels", href: "/insights?view=channels" }, { label: "Products", href: "/products" }],
  ...
  },
  {
    ...
    citations: [{ label: "Recovery Slides", href: "/products/p6" }, { label: "Insights · Channels", href: "/insights?view=channels" }],
...
const DEFAULT_ANSWER = {
  ...
  answer:
    "I reason across your **margins, ad channels, on-site behaviour and period-over-period movement** at once. Ask me something like the suggestions on the left — e.g. *“Can I afford to scale Cloud 7?”* or *“What's eating my profit?”* I'll always cite the data and tell you when I'm not sure. I don't cover inventory, reorders or cash runway — Carter measures paid media and stops at CM2.",
  bullets: [],
  metrics: [],
  citations: [],
...
const INTENT_EXAMPLES = {
  ...
  "scale-cloud7": ["Can I afford to scale Cloud 7?", "Should I put more ad budget into my best seller?", "Is it safe to scale ads on the Cloud 7 bra?"],
...
function buildProductAnswer(p) {
  ...
  const bullets = [];
  if (p.mediaLosing) bullets.push(`Its **ads are underwater** — ${money(p.adSpend)} of spend against ${money(p.paidCm1)} of attributed CM1. Cut the spend or fix the targeting.`);
  ...
  if (p.delta?.cm2.dir === "down") bullets.push(`CM2 is **${money(Math.abs(p.delta.cm2.abs))} down** on the prior period (${money(p.prev.cm2)} → ${money(p.cm2)}).`);
  else if (p.delta?.cm2.dir === "up") bullets.push(`CM2 is **${money(p.delta.cm2.abs)} up** on the prior period — whatever changed is working.`);
  ...
  return {
    ...
    answer: `**${p.name}** earns **${pct(p.cm1Pct)} CM1** and **${pct(p.cm2Pct)} CM2** — ${money(p.cm2PerUnit, { decimals: 2 })} of CM2 per unit. Its media returns **${multiple(p.cmRoas)}** CM-ROAS. It's a **${q.label}**.`,
    bullets,
    metrics,
...
export const STARTER_GROUPS = [
  ...
  },
  {
    ...
    prompts: [
      "Can I afford to scale Cloud 7 — is there headroom left on it?",
      "Which category is dragging the portfolio down?",
      "What changed since last period — where did I gain or lose margin?",
    ],
  },
  {
...
export const FOLLOWUPS = {
  ...
  "scale-cloud7": [
    { label: "What changed?", q: "What changed since last period — where did I gain or lose margin?" },
    { label: "Which category is weakest?", q: "Which category is dragging the portfolio down?" },
  ],
  discount: [
