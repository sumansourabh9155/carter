...
export function buildAllowedNumbers(engines) {
  const { allProducts, productsSummary, marketingData } = engines;
  const set = new Set();
  ...
  for (const p of products) {
    [p.revenue, p.cm1, p.cm2, p.cm3, p.returns, p.cogs, p.adSpend, p.unitCost].forEach(addMoney);
    [p.cm1Pct, p.cm2Pct, p.cm3Pct].forEach(addPct);
    addMult(p.cmRoas); addMult(p.revRoas);
    addInt(p.units);
    if (p.cm2PerUnit != null) { addMoney(p.cm2PerUnit); }
    [p.paidCm1, p.mediaCm2, p.paidRevenue].forEach(addMoney);
    // The prior period is derivable, so every figure Aura can quote about it
    // is legitimate too — otherwise "CM2 fell from $X" reads as invented.
    if (p.prev) {
      [p.prev.revenue, p.prev.cm1, p.prev.cm2, p.prev.adSpend].forEach(addMoney);
      addMult(p.prev.cmRoas);
    }
    if (p.delta) {
      [p.delta.revenue.abs, p.delta.cm1.abs, p.delta.cm2.abs, p.delta.adSpend.abs].forEach(addMoney);
      [p.delta.revenue.pct, p.delta.cm1.pct, p.delta.cm2.pct].forEach(addPct);
      addMult(p.delta.cmRoas.abs);
    }
  }
  ...
  if (s.prev) {
    [s.prev.revenue, s.prev.cm1, s.prev.cm2, s.prev.adSpend].forEach(addMoney);
    addMult(s.prev.cmRoas);
  }
  if (s.delta) {
    [s.delta.revenue.abs, s.delta.cm1.abs, s.delta.cm2.abs, s.delta.adSpend.abs].forEach(addMoney);
    [s.delta.revenue.pct, s.delta.cm1.pct, s.delta.cm2.pct].forEach(addPct);
    addMult(s.delta.cmRoas.abs);
  }

  // Category rollups are quotable too — the board reasons at that level now.
  for (const g of portfolioRollup(products, "category").groups) {
    [g.totals.revenue, g.totals.cm1, g.totals.cm2, g.totals.adSpend].forEach(addMoney);
    addMult(g.totals.cmRoas);
    addInt(g.skuCount);
    if (g.totals.delta) { addMoney(g.totals.delta.cm2.abs); addMult(g.totals.delta.cmRoas.abs); }
    if (g.totals.prev) { addMoney(g.totals.prev.cm2); addMult(g.totals.prev.cmRoas); }
  }
...
export const GOLDEN_SET = [
  ...
  // Replaced the former cash-runway and stockout cases: inventory and
  // treasury left the product, so an eval that still demanded answers about
  // them was gating on behaviour we removed on purpose. These two hold the
  // new ground — the period comparison and the category rollup — which is
  // what the eval is for.
  {
    id: "what-changed", det: false,
    question: "What changed since last period — where did I gain or lose margin?",
    expect: (e) => {
      const s = e.productsSummary();
      return {
        mustContainAny: [money(Math.abs(s.delta?.cm2.abs ?? 0)), money(s.cm2), "last period", "prior"],
        description: "period-over-period CM2 movement",
      };
    },
  },
  {
    id: "worst-category", det: false,
    question: "Which category is dragging the portfolio down?",
    expect: (e) => {
      const groups = portfolioRollup(e.allProducts(), "category").groups;
      const worst = [...groups].sort((a, b) => a.totals.cm2 - b.totals.cm2)[0];
      return {
        mustContainAny: [worst?.label, worst ? money(worst.totals.cm2) : null, "category"].filter(Boolean),
        description: "weakest category by CM2",
      };
    },
  },
  {
