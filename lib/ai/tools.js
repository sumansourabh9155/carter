...
import { allProducts, oneProduct, productsSummary, returnsRanking } from "@/lib/api/mock/products";
...
import { demoLastSync } from "@/lib/connectors/mockSource";
...
// Freshness is read from the connector, not written into a string.
const FRESH_ORDERS = () => `Orders synced ${timeAgo(demoLastSync())}`;
const FRESH_ADS = "Ad data restates for ~24h — directional";
const FRESH_PIXEL = "Carter Web Pixel counts, this period";
...
// cm2Change / cmRoasChange rank by MOVEMENT rather than level — "what got
// worse this period" is a different question from "what is worst", and it is
// the one a media team asks on a Monday.
const RANK_KEYS = ["revenue", "cm1", "cm2", "cm3", "cm1Pct", "cm2Pct", "cmRoas", "cm2Change", "cmRoasChange", "trendPct", "returns", "units"];

// Derived ranking values that don't live on the product as a flat field.
const RANK_VALUE = {
  cm2Change: (p) => p.delta?.cm2.abs ?? null,
  cmRoasChange: (p) => p.delta?.cmRoas.abs ?? null,
};

const RANK_FILTERS = {
  losingMoney: (p) => p.mediaLosing,
  estimated: (p) => p.estimated,
  declining: (p) => p.delta?.cm2.dir === "down",
  none: () => true,
};
...
function compactProduct(p) {
  return {
    ...
    cm2Change: p.delta?.cm2.abs ?? null, cmRoasChange: p.delta?.cmRoas.abs ?? null,
    mediaLosing: p.mediaLosing, losingMoney: p.losingMoney, paidCm1: p.paidCm1, mediaCm2: p.mediaCm2, estimated: p.estimated,
  };
}
...
export const TOOLS = [
  {
    ...
    description: "Store-wide P&L totals (revenue, CM1/CM2/CM3 dollars and %, CM-ROAS, units) with period-over-period movement, plus the store health snapshot (profitability, open action counts). Start here for 'how is the business doing' and 'what changed' questions.",
  ...
  {
    name: "rank_products",
    description: "Rank the catalog by one metric, optionally filtered. Use for 'best/worst/top N' questions — e.g. worst CM2, biggest returns, or (with cm2Change / cmRoasChange) what moved most since the prior period.",
    parameters: {
      type: "object",
      properties: {
        by: { type: "string", enum: RANK_KEYS, description: "Metric to rank by. cm2Change / cmRoasChange are period-over-period movement (ascending = biggest declines first), everything else is this period's level." },
        ...
        filter: { type: "string", enum: ["losingMoney", "estimated", "declining", "none"], description: "Optional pre-filter. declining = CM2 fell versus the prior period." },
      },
      required: ["by"],
    },
    execute({ by = "revenue", direction = "desc", limit = 5, filter = "none" } = {}) {
      ...
      const rows = allProducts()
        .filter(pick)
        .sort((a, b) => {
          const read = RANK_VALUE[by] ?? ((p) => p[by]);
          const va = read(a); const vb = read(b);
          if (va == null && vb == null) return 0;
          if (va == null) return 1; // nulls (no ad spend, no prior period) sink to the bottom
          if (vb == null) return -1;
  ...
  },
  {
    ...
    description:
      "The level ABOVE the SKU: totals grouped by category (or margin quadrant / lifecycle), each with period-over-period movement. Use this for 'which part of the business' questions — which category is weakest, where growth came from, what changed at portfolio level — instead of eyeballing the product list.",
    parameters: {
      type: "object",
      properties: {
        dimension: { type: "string", enum: ["category", "quadrant", "lifecycleStage"], description: "How to group. Defaults to category." },
      },
    },
    execute({ dimension = "category" } = {}) {
      const r = portfolioRollup(allProducts(), dimension);
      const data = {
        ...
        groups: r.groups.map((g) => ({
          ...
          vsPriorPeriod: g.totals.delta
            ? { cm2Change: g.totals.delta.cm2.abs, cm2ChangePct: g.totals.delta.cm2.pct, cmRoasChange: g.totals.delta.cmRoas.abs, priorCm2: g.totals.prev?.cm2 ?? null }
            : null,
          topSku: g.best ? { name: g.best.name, cm2: g.best.cm2 } : null,
          weakestSku: g.worst ? { name: g.worst.name, cm2: g.worst.cm2 } : null,
        })),
        biggestGainer: r.topGainer?.label ?? null,
        biggestDecliner: r.topDecliner?.label ?? null,
      };
      return ok(
        data,
        [{ label: "Products", href: "/products" }, { label: "Insights", href: "/insights" }],
        FRESH_ORDERS(),
        "Group totals are summed through the same margin engine as the store totals, so they reconcile exactly."
      );
    },
  },
  {
    name: "list_insights",
    description: "The prioritized cross-domain insight board: what to scale, cut, or fix, at category and SKU level, ranked by severity and dollar impact. Use for 'what should I fix first / what needs attention' questions.",
    parameters: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    execute({ productId } = {}) {
      ...
      if (!productId) {
        ...
        return ok(data, [{ label: "Insights · Channels", href: "/insights?view=channels" }], FRESH_ADS, "Break-even is 1.0x CM-ROAS — channels below it lose money per dollar.");
      }
      ...
      return ok(
        data,
        [productLink(p), { label: "Insights · Channels", href: "/insights?view=channels" }],
        FRESH_ADS,
  ...
  },
  {
    ...
    execute({ productId } = {}) {
      ...
      if (!productId) {
        ...
        return ok(data, [{ label: "On-site Funnel", href: "/pixel/funnel" }], FRESH_PIXEL);
      }
      const { id, candidates } = resolveToId(productId);
      if (!id) return fail(`No product matched "${productId}".`, candidates.length ? { candidates } : null);
      ...
      return ok(data, [{ label: `${f.name} → breakdown`, href: `/products/${f.id}` }, { label: "On-site Funnel", href: "/pixel/funnel" }], FRESH_PIXEL);
    },
  },
  {
    name: "audience",
  ...
  },
  {
    ...
    execute({ channelId, changePct } = {}) {
      ...
      return ok(data, [{ label: "Insights · Channels", href: "/insights?view=channels" }], FRESH_ADS);
    },
  },
  {
...
export function buildBrandSnapshot() {
  ...
  return {
    ...
    mediaLosingCount: products.filter((p) => p.mediaLosing).length,
