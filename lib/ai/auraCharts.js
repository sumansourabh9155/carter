...
function buildWaterfallData(p) {
  ...
  return [
    ...
    { name: "CM1", value: p.cm1, fill: "#00838f", marker: true },
    { name: "− Ad spend", value: -p.adSpend, fill: "#ffa000" },
    { name: "CM2", value: p.cm2, fill: "#7b1fa2", marker: true },
    { name: "− Overhead", value: -overhead, fill: "#e4eaed" },
...
export function resolveChartRef(ref) {
  ...
  if (kind === "trend" && arg === "revenue") {
    ...
    return {
      ...
      data: trend,
      series: [
        { key: "revenue", name: "Revenue", color: "#2238b0" },
        { key: "cm3", name: "CM3 (net)", color: "#2e7d32" },
      ],
    };
  }
  ...
  if (kind === "trend" && arg === "sessions") {
    return {
      ...
      subtitle: "Last 6 weeks (Carter Web Pixel)",
      xKey: "week", yFormat: "number",
      data: WEEKLY_SESSIONS,
      series: [{ key: "sessions", name: "Sessions", color: "#00838f" }],
    };
  }
  ...
  if (kind === "projection" && arg === "budget") {
    ...
    return {
      ...
      data,
      series: [{ key: "units", name: "Projected orders", color: "#2238b0", dashed: true }],
    };
  }
  ...
  if (kind === "split" && arg === "paidearned") {
    ...
    return {
      ...
      items: [
        { name: "Paid (ads)", value: paid, color: "#2238b0", sub: `${Math.round((paid / web.orders) * 1000) / 10}% of ${web.orders.toLocaleString()} orders` },
        { name: "Earned (organic, direct, email)", value: web.orders - paid, color: "#2e7d32" },
      ],
    };
  }
  ...
  if (kind === "funnel") {
    const web = websiteAnalytics();
    if (arg === "store") {
      return { type: "funnel", title: "Where visitors drop off", subtitle: "Store-wide, this period (Carter Web Pixel)", steps: web.storeFunnel };
    }
    ...
    return {
      ...
      subtitle: "This product's path from view to sale (Carter Web Pixel)",
      steps: [
