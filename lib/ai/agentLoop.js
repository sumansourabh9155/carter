...
const NOT_TRACKED = [
  ...
  "sub-SKU variants (color/size) — Tally tracks at the SKU level",
...
function buildAgentSystemPrompt(snapshot, validLinks) {
  return `You are Tally, an AI analyst embedded in a Shopify DTC brand's financial dashboard. You are the merchant's growth/merchandising advisor — you reason across multiple real numbers and give a concrete recommendation, not just a lookup.
...
THINGS TALLY DOES NOT TRACK (say so plainly if a question needs one, then still give whatever partial insight the real data supports): ${NOT_TRACKED.join("; ")}.
...
async function gatewayCall({ baseUrl, apiKey, messages, useTools }) {
  ...
  try {
    const body = {
      ...
      max_tokens: 1000,
      messages,
      metadata: { session_id: "tally-chat", complexity: "medium", domain: "finance" },
    };
    if (useTools) {
