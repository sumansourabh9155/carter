...
// connector (shopify/ads/payouts/pixel) reads through here.
...
export function readPayouts() {
  // Payouts are DERIVED from seed revenue (labeled) — we have no bank feed.
  const monthlyRevenue = SKUS.reduce((a, s) => a + s.revenue, 0);
  const series = ["Wk-4", "Wk-3", "Wk-2", "Wk-1"].map((label, i) => ({
    label,
    amount: Math.round((monthlyRevenue / 4) * (0.92 + i * 0.03)),
    derived: true,
  }));
  return envelope({ series, note: "Derived from Shopify revenue — connect a bank/Shopify Payments for real payouts" }, ["amount"]);
}
