...
import { paidShareFor } from "@/lib/compute/margin";
...
/*
  Applies the overlay to ONE raw seed record.

  PAUSING ADS COSTS YOU THE SALES THE ADS BOUGHT. This used to zero the spend
  and leave units and revenue untouched, which made every pause look like free
  money: the outcome loop measured +$1,728 on an action the simulator had
  correctly priced at +$1,144, and scored it "511% better than forecast".
  The model was wrong, not the forecast.

  Cutting spend by a fraction removes that same fraction of the SKU's PAID
  demand — its earned orders (organic, direct, email) continue, which is the
  whole reason paidShare exists. Fees and returns scale with the orders that
  no longer happen.

  Shallow-copies only when a multiplier exists, so untouched SKUs keep
...
export function applyOverridesToRawSku(raw) {
  ...
  // Fraction of paid demand withdrawn: cut spend to 0 and you lose all of it,
  // cut it in half and you lose half.
  const lost = paidShare * (1 - Math.min(1, mult));
  const keep = 1 - lost;

  return {
    ...raw,
    adSpend: Math.round(raw.adSpend * mult),
    units: Math.round(raw.units * keep),
    revenue: Math.round(raw.revenue * keep),
    fees: Math.round(raw.fees * keep),
    returns: Math.round(raw.returns * keep),
  };
}
