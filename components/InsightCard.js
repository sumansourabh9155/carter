...
export function InsightCard({ insight }) {
  ...
  const Icon = s.icon;
  return (
    <Card className={cn("flex flex-col gap-3 p-5 transition-colors hover:border-black/15", s.ring)}>
      <div className="flex items-start gap-3">
        <span className={cn("mt-0.5 grid size-9 shrink-0 place-items-center rounded-lg bg-black/[0.04]", s.tone)}>
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
            ...
            {insight.metric && (
              <span className={cn("tabular shrink-0 rounded-md px-2 py-0.5 text-xs font-semibold", s.chip)}>{insight.metric}</span>
            )}
          </div>
